...
function set_database_ssl() {
    ...
    if [ "${DATABASE_HOSTNAME}" == "false" ] ; then
		...
		#POST_USER="$(grep -c '^postgres:' /etc/passwd)"
		if ! grep -q '^postgres:' /etc/passwd ; then
			echo 'test3'
			sudo useradd -u 70 postgres
		fi
		sudo chown postgres:postgres ${KASM_INSTALL_BASE}/certs/db_server.key
...
function base_install() {
    ...
    if [ -f ${KASM_INSTALL_BASE}/certs/db_server.key ]; then 
    	chown postgres:postgres ${KASM_INSTALL_BASE}/certs/db_server.key
    fi
    ...
    then
        ...
        ${KASM_INSTALL_BASE}/bin//utils/db_init -s ${KASM_INSTALL_BASE}/conf/database/seed_data/default_images.yaml -q ${DATABASE_HOSTNAME} -T ${DATABASE_PORT} -Q ${DEFAULT_DATABASE_PASSWORD} -t ${DATABASE_SSL}
...
function pull_images() {
	...
	echo "Loading Images"
        for image in ${KASM_RELEASE}/images/*.tar ; do
...
while getopts 'etdhvDZp:P:q:m:n:i:r:U:Q:L:z:R:S:t:T:o:M:' flag; do
  case "${flag}" in
    e)
        ACCEPT_EULA='true'
        ;;
